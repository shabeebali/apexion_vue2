...
<v-row>
	<v-col class="mx-4">
		<v-toolbar dense color="transparent" flat>
			...
			<v-toolbar-items>
				<v-btn v-if="meta.create == 'true'" color="primary" dense depressed :href="baseUrl+'/admin/taxonomies/add'">Create</v-btn>
			</v-toolbar-items>
		</v-toolbar>
		<v-card class="mt-2">
			<v-card-text>
	...
	<v-dialog v-model="confirmDialog" persistent width="500">
		<v-card>
			...
			<v-card-text>
				...
				Do you really Want to continue?. All the categories values under this taxonomy will be deleted as well.</v-alert>
			</v-card-text>
			<v-card-actions>
...
<script>
    new Vue({
        ...
        methods:{
			...
			edit(id){
				window.location.href = this.baseUrl+'/admin/taxonomies/edit/'+id
			},
			deleteConfirm(id){
				this.delete_id = id
