	        ...
	        <v-card-text>
				<v-row class="mx-4">
	        		...
	        		<v-col cols="12" md="4">
						...
						<v-switch disabled v-model="fd.in_pc.value" true-value="1" false-value="0" label="Include in Product Code?"></v-switch>
						<v-select disabled label="Code Length" v-model="fd.code_length.value" :items="code_length.options" v-on:change="cTypeUpd"></v-select>
						<v-switch disabled v-model="fd.autogen.value" true-value="1" false-value="0" label="Auto Generate?"></v-switch>
						<v-select disabled v-model="fd.code_type.value" label="Code Type" :items="code_type[fd.code_length.value]['options']"></v-select>
					</v-col>
					<v-col cols="12" md="4"></v-col>
				</v-row>
			</v-card-text>
    ...
    new Vue({
        ...
        mounted(){
        	...
        	axios.all([
	            ...
	            axios.get('taxonomies/{{$id}}').then((response)=>{
